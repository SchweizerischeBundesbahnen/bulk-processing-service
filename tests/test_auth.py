...
@pytest.fixture
def service_client(tmp_path) -> TestClient:
    """Client for the real service, with a working job store."""
...
def test_merge_endpoints_require_key(monkeypatch: pytest.MonkeyPatch, service_client: TestClient) -> None:
    """The merge endpoints reject unauthenticated requests, probes and version stay open."""
    monkeypatch.setenv(API_KEY_ENV_VAR, "secret")
    assert service_client.post("/api/convert/start", json={}).status_code == 401
    assert service_client.get("/health").status_code == 200
    assert service_client.get("/version").status_code == 200


def test_merge_endpoints_accept_valid_key(monkeypatch: pytest.MonkeyPatch, service_client: TestClient) -> None:
    monkeypatch.setenv(API_KEY_ENV_VAR, "secret")
    assert service_client.post("/api/convert/start", json={}, headers={"X-API-Key": "secret"}).status_code == 201
    assert service_client.post("/api/convert/start", json={}, headers={"Authorization": "Bearer secret"}).status_code == 201


def test_merge_endpoints_open_without_key(monkeypatch: pytest.MonkeyPatch, service_client: TestClient) -> None:
    monkeypatch.delenv(API_KEY_ENV_VAR, raising=False)
    assert service_client.post("/api/convert/start", json={}).status_code == 201
